...
if($_GET['dari_bulan'] OR $_GET['sampai_bulan']){
if(empty($_GET['th'] AND $_GET['kej'])){ 
$html .= ' <div>Semua kejadian Kedaruratan</div> '; }else{
...
$html .= ' <tr>
           <th>No.</th><th>Kejadian</th><th>Kecamatan</th><th>Desa</th><th>Alamat</th><th>Tanggal Terima</th><th>Tanggal Selesai</th><th>Keterangan</th><th>Foto</th>  
                          ...
                          if($_GET['dari_bulan'] AND $_GET['th']){ $tampil = mysqli_query($kominfo, "select * from lokasi  where  bulan='$dari_bulan' and tahun='$th'  order by id desc "); }
                          if($_GET['dari_bulan'] AND $_GET['th'] AND $_GET['kej']){ $tampil = mysqli_query($kominfo, "select * from lokasi  where  bulan='$dari_bulan' and tahun='$th' and kejadian='$kej' order by id desc "); }
                          if($_GET['dari_bulan'] AND $_GET['sampai_bulan'] AND $_GET['th']){ $tampil = mysqli_query($kominfo, "select * from lokasi  where bulan between '$dari_bulan' and '$sampai_bulan' and tahun='$th'  order by id desc "); }
                          if($_GET['dari_bulan'] AND $_GET['sampai_bulan'] AND $_GET['th'] AND $_GET['kej']){ $tampil = mysqli_query($kominfo, "select * from lokasi  where bulan between '$dari_bulan' and '$sampai_bulan' and tahun='$th' and kejadian='$kej' order by id desc "); }
...
$id_kec=$hasil['kec']; $kec1 = mysqli_query($kominfo, "select * from kecamatan where id='$id_kec'"); $kec2 = mysqli_fetch_array($kec1);
$html .= ''.$kec2['nama_kecamatan'].'</td>
           <td valign="top" width="10%">';
$id_desa=$hasil['desa'];$desa1 = mysqli_query($kominfo, "select * from desa where id='$id_desa'");   $desa2 = mysqli_fetch_array($desa1);
...
$html .= '<tr style="background-color: #eee!important;">      
...
$dari_bulan = $_GET['dari_bulan']; $sampai_bulan = $_GET['sampai_bulan'];  $th = $_GET['th']; $kej = $_GET['kej']; 
                          if($_GET['dari_bulan'] AND $_GET['th']){ $data = $_GET['dari_bulan']."_".$_GET['th']; }
                          if($_GET['dari_bulan'] AND $_GET['th'] AND $_GET['kej']){ $data = $_GET['dari_bulan']."_".$_GET['kej']; }
                          if($_GET['dari_bulan'] AND $_GET['sampai_bulan'] AND $_GET['th']){ $data = $_GET['dari_bulan']."_".$_GET['sampai_bulan']."_".$_GET['th']."_semua_kejadian"; }
                          if($_GET['dari_bulan'] AND $_GET['sampai_bulan'] AND $_GET['th'] AND $_GET['kej']){ $data = $_GET['dari_bulan']."_".$_GET['sampai_bulan']."_".$_GET['th']."_".$_GET['kej']; }


if(empty($_GET)){ $data="kab_sumenep";}
