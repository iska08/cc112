describe('test_data_inputdatauser', () => {
    ...
    it('input data user', () => {
      ...
      cy.get('#form_tambah_user > :nth-child(3) > .form-control').type('08121222')//tambah no telp
      cy.get('#form_tambah_user > :nth-child(4) > .form-control').type('polisi@gmail.com') //tambah gmail
      cy.get('#hak_akses').select('Tim') //pilih hak akses
      cy.get(':nth-child(2) > .form-check-input').click()
      cy.get('#form_tambah_user > :nth-child(7) > .btn').click()
      cy.get('.swal2-confirm').click()
      cy.get('#user_filter > label > .form-control').type('Polisi')
      cy.get(':nth-child(1) > [width="5%"] > .btn-group > #hapus_user').click()
      cy.get('.swal2-confirm').click()
      cy.get('.swal2-confirm').click()
